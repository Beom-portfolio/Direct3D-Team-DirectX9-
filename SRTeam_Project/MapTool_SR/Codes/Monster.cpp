...
const _vec3 & CMonster::GetMonPos(void)
{
...
void CMonster::Render_GameObject(void)
{
	...
	m_pTextureCom->SetUp_OnGraphicDev(0, m_iMonType);
	m_pBufferCom->Render_Buffer();

	m_pGraphicDev->SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);
}

void CMonster::TransPos_Mon(const _vec3 * vPos)
{
	m_pTransformCom->Set_Infomation(Engine::CTransform::INFO_POSITION, vPos);
}

void CMonster::Billboard_Mon(void)
{
	_matrix			matView;
	matView.Identity();
	...
	Scaling_Mon(m_vScale.x, m_vScale.y, m_vScale.z);
}

void CMonster::Scaling_Mon(const _float & fX, const _float & fY, const _float & fZ)
{
...
HRESULT CMonster::Ready_Component(void)
{
	...
	// For.Buffers
	pComponent = m_pBufferCom = (Engine::CRect_Cube*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_LOGO, L"Component_Buffer_Rect");
	if (nullptr == pComponent)
		return E_FAIL;
	...
	// For.Texture
	pComponent = m_pTextureCom = (Engine::CTexture*)CComponent_Manager::GetInstance()->Clone_Component(SCENE_LOGO, L"Component_Texture_Monster");
	if (nullptr == pComponent)
		return E_FAIL;
...
CMonster * CMonster::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	...
	{
		MSG_BOX("Monster Created Failed");
