...
CCamera::CCamera(LPDIRECT3DDEVICE9 pGraphicDev)
	...
	, m_Camera_Type(CAMERA_END)
{
	m_matWorld.Identity();
	m_matView.Identity();
	m_matProj.Identity();
	m_TempViewmat.Identity();
...
void CCamera::Setting_Camera(void)
{
	SetUp_ViewMatrix(&m_matView);
	SetUp_ProjMatrix(&m_matProj);
}
...
void CCamera::SetUp_ViewMatrix(_matrix* pMatrix)
{
	...
	//m_TempViewmat = m_matWorld;

	memcpy(&m_TempViewmat.m[0][0], &m_matWorld.m[0][0], sizeof(_vec3));
	memcpy(&m_TempViewmat.m[1][0], &m_matWorld.m[1][0], sizeof(_vec3));
	memcpy(&m_TempViewmat.m[2][0], &m_matWorld.m[2][0], sizeof(_vec3));
	memcpy(&m_TempViewmat.m[2][0], &m_matWorld.m[3][0], sizeof(_vec3));
