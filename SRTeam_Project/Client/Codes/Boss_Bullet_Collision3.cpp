#include "stdafx.h"
#include "..\Headers\Boss_Bullet_Collision3.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CBoss_Bullet_Collision3::CBoss_Bullet_Collision3(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CBoss_Bullet_Collision3::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CBoss_Bullet_Collision3::Update_GameObject(const _float & fTimeDelta)
{
	...
	if (m_vStartPos.x + 10.f <= m_vPos.x || m_vStartPos.x - 10.f >= m_vPos.x || m_vStartPos.z + 10.f <= m_vPos.z || m_vStartPos.z - 10.f >= m_vPos.z)
		m_tInfo.iHp = 0;
...
_int CBoss_Bullet_Collision3::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CBoss_Bullet_Collision3::Render_GameObject(void)
{
...
HRESULT CBoss_Bullet_Collision3::Ready_Component()
{
	...
	//나중에 보스 불렛으로 바꾸자
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Stage2Boss_Bullet4"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_Boss_Bullet4_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

CBoss_Bullet_Collision3 * CBoss_Bullet_Collision3::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CBoss_Bullet_Collision3* pInstance = new CBoss_Bullet_Collision3(pGraphicDev, eStage);
	if (FAILED(pInstance->Ready_GameObject(vPos, vLook)))
	{
		MSG_BOX("CBoss_Bullet_Collision3 Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CBoss_Bullet_Collision3::Free()
{
	_ulong		dwRefCnt = 0;
