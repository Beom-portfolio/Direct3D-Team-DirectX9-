#include "stdafx.h"
#include "..\Headers\Monster_NormalSordier.h"
...
CMonster_NormalSordier::CMonster_NormalSordier(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vStartPos, SCENETYPE eStage)
...
HRESULT CMonster_NormalSordier::Ready_GameObject(void)
{
...
_int CMonster_NormalSordier::Update_GameObject(const _float & fTimeDelta)
{
...
_int CMonster_NormalSordier::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_bDieing = true;
...
void CMonster_NormalSordier::Render_GameObject(void)
{
...
HRESULT CMonster_NormalSordier::Ready_Component(void)
{
	...
	// Texture_Walk
	pComponent = m_pWalkTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(m_eStage, L"Component_Texture_Walk_NormalArmy");
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Texture_Walk_NormalArmy", pComponent));
	m_pWalkTextureCom->AddRef();

	// Texture_Attack
	pComponent = m_pAttackTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(m_eStage, L"Component_Texture_NormalArmy_Attack");
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Texture_NormalArmy_Attack", pComponent));
	m_pAttackTextureCom->AddRef();

	// Texture_Death
	pComponent = m_pDeathTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(m_eStage, L"Component_Texture_NormalArmy_Death");
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Texture_NormalArmy_Death", pComponent));
	m_pDeathTextureCom->AddRef();

	// Texture_Hit
	pComponent = m_pHitTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(m_eStage, L"Component_Texture_NormalArmy_Hit");
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Texture_NormalArmy_Hit", pComponent));
...
void CMonster_NormalSordier::Move_OnTerrain(void)
{
...
void CMonster_NormalSordier::PlayerPos_Check(float fTimeDelta)
{
	CComponent* pPlayerCom = m_pObject_Manager->Get_Component(SCENE_STATIC, L"Layer_Player", 0, L"Transform_com");
	_vec3 vPlayerPos = *dynamic_cast<CTransform*>(pPlayerCom)->Get_Infomation(CTransform::INFO_POSITION);
...
HRESULT CMonster_NormalSordier::Create_Bullet(const _tchar * pLayertag, float fTimeDelta)
{
	return E_NOTIMPL;
}

CMonster_NormalSordier * CMonster_NormalSordier::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vStartPos, SCENETYPE eStage)
{
	CMonster_NormalSordier *	pInstance = new CMonster_NormalSordier(pGraphicDev, vStartPos, eStage);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CMonster_NormalSordier Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CMonster_NormalSordier::Free(void)
{
	_ulong		dwRefcnt = 0;
