#include "stdafx.h"
#include "..\Headers\Boss_Bullet_Collision2.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CBoss_Bullet_Collision2::CBoss_Bullet_Collision2(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CBoss_Bullet_Collision2::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CBoss_Bullet_Collision2::Update_GameObject(const _float & fTimeDelta)
{
...
_int CBoss_Bullet_Collision2::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CBoss_Bullet_Collision2::Render_GameObject(void)
{
...
HRESULT CBoss_Bullet_Collision2::Ready_Component()
{
	...
	//나중에 보스 불렛으로 바꾸자
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Stage1Boss_Bullet2"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_Boss_Bullet1_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

CBoss_Bullet_Collision2 * CBoss_Bullet_Collision2::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CBoss_Bullet_Collision2* pInstance = new CBoss_Bullet_Collision2(pGraphicDev, eStage);
...
_ulong CBoss_Bullet_Collision2::Free()
{
	_ulong		dwRefCnt = 0;
