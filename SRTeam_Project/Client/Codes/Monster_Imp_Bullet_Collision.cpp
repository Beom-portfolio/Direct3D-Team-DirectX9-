#include "stdafx.h"
#include "..\Headers\Monster_Imp_Bullet_Collision.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CMonster_Imp_Bullet_Collision::CMonster_Imp_Bullet_Collision(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CMonster_Imp_Bullet_Collision::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CMonster_Imp_Bullet_Collision::Update_GameObject(const _float & fTimeDelta)
{
...
_int CMonster_Imp_Bullet_Collision::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CMonster_Imp_Bullet_Collision::Render_GameObject(void)
{
...
HRESULT CMonster_Imp_Bullet_Collision::Ready_Component()
{
	...
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Imp_Bullet"));
	if (nullptr == pComponent)
		return E_FAIL;
...
CMonster_Imp_Bullet_Collision * CMonster_Imp_Bullet_Collision::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CMonster_Imp_Bullet_Collision* pInstance = new CMonster_Imp_Bullet_Collision(pGraphicDev, eStage);
...
_ulong CMonster_Imp_Bullet_Collision::Free()
{
	_ulong		dwRefCnt = 0;
