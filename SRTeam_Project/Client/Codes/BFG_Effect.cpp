#include "stdafx.h"
#include "..\Headers\BFG_Effect.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CBFG_Effect::CBFG_Effect(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CBFG_Effect::Ready_GameObject(const _vec3 * vPos)
{
...
_int CBFG_Effect::Update_GameObject(const _float & fTimeDelta)
{
...
_int CBFG_Effect::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CBFG_Effect::Render_GameObject(void)
{
...
HRESULT CBFG_Effect::Ready_Component()
{
	...
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_BFG_Effect"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_BFG_Effect_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

CBFG_Effect * CBFG_Effect::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos)
{
	CBFG_Effect* pInstance = new CBFG_Effect(pGraphicDev);
