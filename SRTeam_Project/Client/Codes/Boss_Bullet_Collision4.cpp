#include "stdafx.h"
#include "..\Headers\Boss_Bullet_Collision4.h"

#include "Object_Manager.h"
#include "Component_Manager.h"
#include "Bullet_Effect_Green.h"

CBoss_Bullet_Collision4::CBoss_Bullet_Collision4(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CBoss_Bullet_Collision4::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
	...
	m_tInfo.iAtk = 0;   //이펙트가 데미지

	return NOERROR;

}

_int CBoss_Bullet_Collision4::Update_GameObject(const _float & fTimeDelta)
{
	...
	if (m_vStartPos.x + 6.f <= m_vPos.x || m_vStartPos.x - 6.f >= m_vPos.x || m_vStartPos.z + 6.f <= m_vPos.z || m_vStartPos.z - 6.f >= m_vPos.z)
		m_tInfo.iHp = 0;
...
_int CBoss_Bullet_Collision4::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
	...
	{
		Create_Effect(L"Monster_Bullet");
		return 1;
	}
...
void CBoss_Bullet_Collision4::Render_GameObject(void)
{
...
HRESULT CBoss_Bullet_Collision4::Ready_Component()
{
	...
	//나중에 보스 불렛으로 바꾸자
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Stage2Boss_Bullet3"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_Boss_Bullet3_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

void CBoss_Bullet_Collision4::Create_Effect(const _tchar * pLayertag)
{
	...
	pGameObject = CBullet_Effect_Green::Create(m_pGraphicDev, &StartPos, m_eStage);
...
CBoss_Bullet_Collision4 * CBoss_Bullet_Collision4::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CBoss_Bullet_Collision4* pInstance = new CBoss_Bullet_Collision4(pGraphicDev, eStage);
...
_ulong CBoss_Bullet_Collision4::Free()
{
	_ulong		dwRefCnt = 0;
