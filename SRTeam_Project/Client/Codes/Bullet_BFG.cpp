#include "stdafx.h"
#include "..\Headers\Bullet_BFG.h"

#include "Object_Manager.h"
#include "Component_Manager.h"
#include "BFG_Splash.h"

CBullet_BFG::CBullet_BFG(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CBullet_BFG::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CBullet_BFG::Update_GameObject(const _float & fTimeDelta)
{
...
_int CBullet_BFG::LastUpdate_GameObject(const _float & fTimeDelta)
{
	...
	if (true == m_tInfo.bDie)
	{
		...
		pGameObject = CBFG_Splash::Create(m_pGraphicDev, m_pTransformCom->Get_Infomation(Engine::CTransform::INFO_POSITION));
		
		if (nullptr == pGameObject)
			return 1;

		
		if (FAILED(m_pObjectManager->Ready_GameObject(SCENE_STATIC, L"Bullet", pGameObject)))
			return 1;
		return 1;
	}
	return 0;
...
void CBullet_BFG::Render_GameObject(void)
{
...
HRESULT CBullet_BFG::Ready_Component()
{
	...
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Texture_BFGBullet"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_BFGBullet_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

CBullet_BFG * CBullet_BFG::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook)
{
	CBullet_BFG* pInstance = new CBullet_BFG(pGraphicDev);
	if (FAILED(pInstance->Ready_GameObject(vPos, vLook)))
	{
		MSG_BOX("CBullet_BFG Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CBullet_BFG::Free()
{
	_ulong		dwRefCnt = 0;
