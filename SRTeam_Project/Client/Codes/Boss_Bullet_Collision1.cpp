#include "stdafx.h"
#include "..\Headers\Boss_Bullet_Collision1.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CBoss_Bullet_Collision1::CBoss_Bullet_Collision1(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CBoss_Bullet_Collision1::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CBoss_Bullet_Collision1::Update_GameObject(const _float & fTimeDelta)
{
...
_int CBoss_Bullet_Collision1::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CBoss_Bullet_Collision1::Render_GameObject(void)
{
...
HRESULT CBoss_Bullet_Collision1::Ready_Component()
{
	...
	//나중에 보스 불렛으로 바꾸자
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Stage1Boss_Bullet1"));
	if (nullptr == pComponent)
		return E_FAIL;
...
CBoss_Bullet_Collision1 * CBoss_Bullet_Collision1::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CBoss_Bullet_Collision1* pInstance = new CBoss_Bullet_Collision1(pGraphicDev, eStage);
...
_ulong CBoss_Bullet_Collision1::Free()
{
	_ulong		dwRefCnt = 0;
