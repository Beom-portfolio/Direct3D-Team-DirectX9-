#include "stdafx.h"
#include "..\Headers\Monster_Afrit_Bullet_Collision.h"

#include "Object_Manager.h"
#include "Component_Manager.h"

CMonster_Afrit_Bullet_Collision::CMonster_Afrit_Bullet_Collision(LPDIRECT3DDEVICE9 pGraphicDev, SCENETYPE eStage)
...
HRESULT CMonster_Afrit_Bullet_Collision::Ready_GameObject(const _vec3 * vPos, const _vec3 * vLook)
{
...
_int CMonster_Afrit_Bullet_Collision::Update_GameObject(const _float & fTimeDelta)
{
...
_int CMonster_Afrit_Bullet_Collision::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (0 >= m_tInfo.iHp)
		m_tInfo.bDie = true;
...
void CMonster_Afrit_Bullet_Collision::Render_GameObject(void)
{
...
HRESULT CMonster_Afrit_Bullet_Collision::Ready_Component()
{
	...
	//Texture
	pComponent = m_pTextureCom = DCAST(Engine::CTexture*, CComponent_Manager::GetInstance()->Clone_Component(m_eStage, L"Component_Texture_Afrit_Bullet"));
	if (nullptr == pComponent)
		return E_FAIL;
	m_mapComponent[Engine::CComponent::TYPE_STATIC].insert(MAPCOMPONENT::value_type(L"Texture_Afrit_Bullet_com", pComponent));
	m_pTextureCom->AddRef();

	return NOERROR;
}

CMonster_Afrit_Bullet_Collision * CMonster_Afrit_Bullet_Collision::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 * vPos, const _vec3 * vLook, SCENETYPE eStage)
{
	CMonster_Afrit_Bullet_Collision* pInstance = new CMonster_Afrit_Bullet_Collision(pGraphicDev, eStage);
	if (FAILED(pInstance->Ready_GameObject(vPos, vLook)))
	{
		MSG_BOX("CMonster_Afrit_Bullet_Collision Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CMonster_Afrit_Bullet_Collision::Free()
{
	_ulong		dwRefCnt = 0;
